...
let sectionNames = ["Warmup", "Passerines", "Forest birds", "Songbirds", "Birds of prey", "Seabirds"]
...
for(let i = 0; i < birdsDataEn.length; i++) {
  ...
  sectionHeading.textContent = sectionNames[i];
  ...
  for(let item of birdsDataEn[i]) {
    ...
    function playAudio() {
      birdAudio.play();
      isPlaying = true;
      birdPlaypause.classList.remove("play");
      birdPlaypause.classList.add("pause");
      birdPlaypause.src = "img/icons/pause.svg";
    }

    function pauseAudio() {
      birdAudio.pause();
      isPlaying = false;
      birdPlaypause.classList.remove("pause");
      birdPlaypause.classList.add("play");
      birdPlaypause.src = "img/icons/play.svg";
    }

    function slide() {
      let slideTo = birdAudio.duration * (birdSlider.value / 100);
      birdAudio.currentTime = slideTo;
    }

    function setUpdate() {
      let sliderPosition = 0;
      if(!isNaN(birdAudio.duration)) {
        sliderPosition = birdAudio.currentTime * (100 / birdAudio.duration);
        birdSlider.value = sliderPosition;

        let currentMinutes = Math.floor(birdAudio.currentTime / 60);
        let currentSeconds = Math.floor(birdAudio.currentTime - currentMinutes * 60);
        let durationMinutes = Math.floor(birdAudio.duration / 60);
        let durationSeconds = Math.floor(birdAudio.duration - durationMinutes * 60);
        ...
        birdDurationCurrent.textContent = currentMinutes + ":" + currentSeconds;
        birdDurationTotal.textContent = durationMinutes + ":" + durationSeconds;
      }
    }
    ...
    birdSlider.addEventListener("change", slide)
  }
}
